...
function mountDrawer(open = true) {
    return mount(AppDrawer, {
        props: { open, title: 'Детали задачи' },
...
describe('AppDrawer', () => {
    it('renders nothing while closed', () => {
        expect(mountDrawer(false).find('[role="dialog"]').exists()).toBe(false);
    });

    it('exposes the dialog semantics and labels it by the title', () => {
        const dialog = mountDrawer().find('[role="dialog"]');
    ...
    it('moves focus into the panel when it opens', async () => {
        const wrapper = mount(AppDrawer, {
            props: { open: false, title: 'Детали задачи' },
            slots: { default: '<button type="button" id="inner">Внутри</button>' },
            attachTo: document.body,
        });
